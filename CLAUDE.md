...
| 行号 | 内容 | 字号 | 颜色 | 说明 |
|------|------|------|------|------|
| 第1行 | 公历日期（1-31） | 12sp | #2C1810 | 墨褐色，周末/节日红色 |
| 第2行 | 农历/节气/节日 | 8sp | #8B7355 | 浅棕色，节气绿色，初一十五红色 |
| 第3行 | 天干 + 十神 | 10sp | #2C1810 | 墨褐色，如"壬 正官" |
| 第4行 | 地支 + 地支十神 | 10sp | #5A4A3A | 深棕色，如"午 比肩" |

### 布局特性

- 格子高度：根据内容自适应（行高80-90dp）
- 同一行所有单元格高度一致
- 今日标识：背景渐变（#C84A3E → #A33D33），文字白色
- 其他月日期：文字半透明
...
见 `docs/design/ui-design.md` 月历页（页面2）
